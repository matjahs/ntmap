...
import app.common
from app.netbox_client import NetBoxError, get_client_from_config
...
# Returns JSON with all objects of given map: devices, interfaces and links between them
# Input: (int) map id
# Output: JSON
def getMap(id):
    ...
    if selectedMap:
        ...
        try:
            netbox = get_client_from_config(app.common.config)
        except NetBoxError as err:
        ...
        for level in goodLevelsDict.keys():
            ...
            for namePattern in goodLevelsDict[level]:
                try:
                    devices = netbox.find_devices_by_name_pattern(namePattern)
                except NetBoxError as err:
                    return { "result": str(err) }
            ...
            for namePattern in goodLevelsDict[level]:
                try:
                    providers = netbox.find_providers_by_name_pattern(namePattern)
                except NetBoxError as err:
                    return { "result": str(err) }
        ...
        map_device_ids = {
            d["netbox_id"]
            for d in graphJson["results"]["nodes"]
            if d.get("class") == "devices"
        }
        for node in graphJson["results"]["nodes"]:
            if node["class"] == "circuits" and node["type"] == "provider":
                try:
                    provider_circuits = netbox.find_circuits_for_provider(
                        node,
                        map_device_ids,
                        float(node["group"]) + 0.5,
                    )
                except NetBoxError as err:
                    return { "result": str(err) }
                circuits.extend(provider_circuits)
        ...
        for node in graphJson["results"]["nodes"]:
            if node["class"] == "devices":
                try:
                    nodeInterfaces = netbox.get_device_interfaces(node["id"], node["netbox_id"])
                except NetBoxError as err:
                    return { "result": str(err) }
