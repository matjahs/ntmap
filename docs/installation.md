...
- Netbox 3.x / 4.x (REST API)
- Python 3.x
- PostgreSQL (Ntmap metadata only)
- Gunicorn
- NGINX
- D3.js
...
Ntmap fetches topology data from the **NetBox REST API** using a token (not NetBox’s database). Set `url` and `token` under `[netbox]` in settings.ini.

```
# cd /opt/ntmap/backend/app
# cp example.settings.ini settings.ini
# nano settings.ini  # set Ntmap DB connect string and NetBox url/token
```
...
```
(venv) # pip3 install psycopg2-binary gunicorn requests
(venv) # deactivate
```
